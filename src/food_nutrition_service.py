...
class FoodNutritionService:
    ...
    def __init__(self, data_dir: str):
        ...
        if self.offline:
            log_info("MCP_OFFLINE enabled: skip FoodNutritionService scheduler and updates.")
            if not os.path.exists(self.db_path):
                log_error(
                    "Food Nutrition DB not found in offline mode. Provide data/food_nutrition.db."
                )
        else:
    ...
    def _update_all_data(self):
        ...
        if self.offline:
            log_info("MCP_OFFLINE enabled: skip FoodNutritionService update.")
            return
